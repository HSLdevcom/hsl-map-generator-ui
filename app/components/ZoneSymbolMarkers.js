...
const getSymbolSize = (symbolSize, mapWidth, mapHeight, mapSelectionSize) => {
    let size = mapSelectionSize.get("size")
    ...
    const mapSelectionSizeDiameter =
        (size.get(0) + size.get(0)) / 2;

    return (mapSelectionSizeDiameter / symbolToMapRatio) * SCALE;
};

const ZoneSymbolMarkers = ({ map, zoneSymbols, updateSymbol, symbolSize, mapWidth, mapHeight, mapSelectionSize }) => {
    ...
    useEffect(() => {
        ...
        const svgSize = getSymbolSize(symbolSize, mapWidth, mapHeight, mapSelectionSize);
        zoneSymbols.forEach(symbol => {
            const markerElement = document.createElement('div');
            ReactDOM.render(getZoneIcon(symbol.get("zone"), svgSize), markerElement);
    ...
    useEffect(() => {
        ...
        const resizeMarkers = () => {
            const newSvgSize = getSymbolSize(symbolSize, mapWidth, mapHeight, mapSelectionSize);
            markersRef.current.forEach(marker => {
                const element = marker.getElement();
                if (marker.symbol) {
                    ReactDOM.render(getZoneIcon(marker.symbol.get("zone"), newSvgSize), element);
                }
            });
        };
