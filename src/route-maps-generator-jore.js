...
const stops = JSON.parse(readFileSync('./stops2.geojson')).features;
...
function renderRoute(routeId) {
  return (callback) => {
    ...
    if (stops1) {
      baseFile.sources.stops1 = {type: 'geojson', data: {type: 'FeatureCollection', features: stops1}};
    }

    if (stops2) {
      baseFile.sources.stops2 = {type: 'geojson', data: {type: 'FeatureCollection', features: stops2}};
    }
    ...
    writeFileSync(`./route-maps/${routeId}.json`, toJSON(baseFile));
    writeFileSync(`./route-maps/${routeId}.style.json`, JSON.stringify(styleFromLayers(baseFile.layers, baseFile.sources).toJS()));
    imageGenerator(({data}) => {
      writeFileSync(`./route-maps/${routeId}.png`, data);
      callback(null, 'done');
    }, {
